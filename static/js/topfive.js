...
function renderBookList(data) {
  ...
  if (data.length === 0) {
    listElement.innerHTML =
      '<li class="empty-state">Drag books here to start your Top Five!</li>';
    return;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function createDropIndicator() {
    if (!dropIndicator) {
      ...
      dropIndicator.style.cssText =
        "height: 2px; background: var(--primary-light, #4A90E2); margin: 4px 0; transition: all 0.2s;";
    }
  ...
  topFiveList.addEventListener("drop", (e) => {
    ...
    if (draggedItem) {
      ...
      const existingIndex = currentRecsList.findIndex(
        (book) => book[0] == newBookId
      );
      ...
      console.log(
        `Added Book ID ${newBookId} at position ${insertPos + 1}. New data:`,
        currentRecsList
      );
    }
  });
